...
export function RentHistoryRow({ agreement }: { agreement: RentalAgreement }) {
  ...
  if (editing) {
    return (
      <form onSubmit={onSave} className="py-2 border-b border-gray-50 dark:border-gray-800 last:border-0 space-y-2">
        <div className="flex items-center gap-2">
          <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
            className="flex-1 border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required />
          <input type="number" step="0.01" value={amount} onChange={e => setAmount(e.target.value)}
            className="w-24 border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 rounded-lg px-2 py-1 text-sm text-right focus:outline-none focus:ring-2 focus:ring-blue-500" required />
        </div>
        ...
        {isIndexRent && (
          <div>
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Basis-Indexstand (VPI)</label>
            <input type="number" step="0.001" value={indexBaseValue} onChange={e => setIndexBaseValue(e.target.value)}
              className="w-32 border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        )}
        <div>
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Beleg (Vertrag/Erhöhungsschreiben)</label>
          <input
            type="file"
            ...
            className="w-full text-xs dark:text-gray-300"
          />
          {agreement.file_url && !file && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">✓ Beleg bereits hinterlegt</p>}
        </div>
        <div className="flex gap-2">
          <button type="submit" disabled={saving} className="text-xs text-blue-600 dark:text-blue-400 hover:underline whitespace-nowrap">
            {saving ? '...' : 'Sichern'}
          </button>
          <button type="button" onClick={() => setEditing(false)} className="text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 whitespace-nowrap">
            Abbrechen
          </button>
        </div>
      </form>
  ...
  return (
    <div className="flex justify-between items-center text-sm py-1.5 border-b border-gray-50 dark:border-gray-800 last:border-0">
