  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-2">
        ...
        {!open && <button onClick={() => setOpen(true)} className="text-sm text-blue-600 dark:text-blue-400 hover:underline whitespace-nowrap">+ Indexstand erfassen</button>}
      </div>
      ...
      {open && (
        <form onSubmit={onSubmit} className="grid grid-cols-2 gap-3 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 mt-3">
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Monat *</label>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Für welchen Monat gilt der VPI-Wert unten? Nicht der Einzugsmonat eines Mieters – das hier ist die allgemeine Indextabelle, aus der sich der Basiswert bei jedem indexgemieteten Vertrag automatisch bedient.</p>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Indexstand *</label>
        ...
        <div className="mt-3 space-y-1">
          {sorted.slice(0, 6).map(r => (
            <div key={r.id} className="flex justify-between text-sm text-gray-600 dark:text-gray-300">
              ...
              <span className="font-medium text-gray-900 dark:text-gray-100">{r.value.toFixed(3)}</span>
            </div>
          ))}
        </div>
      )}
