...
const statusBadge: Record<Status, { label: string; className: string }> = {
  over: { label: 'über Vergleichsmiete', className: 'bg-red-100 text-red-800' },
  under: { label: 'Erhöhungspotential', className: 'bg-blue-100 text-blue-800' },
  within: { label: 'im Rahmen', className: 'bg-green-100 text-green-800' },
  unknown: { label: 'keine Vergleichsmiete hinterlegt', className: 'bg-gray-100 text-gray-500' },
}
      ...
      <table className="w-full text-sm min-w-[640px]">
        ...
        <tbody>
          {rows.map(row => {
            ...
            return (
              <tr key={row.tenant.id} className="border-b border-gray-50 last:border-0">
                <td className="py-2.5">
                  <Link href={`/tenants/${row.tenant.id}`} className="font-medium text-gray-900 hover:text-blue-700">
                    {row.tenant.name}
                  </Link>
                </td>
                <td className="py-2.5 text-gray-500 text-xs">{propertyLabel(row.property)}</td>
                <td className="py-2.5 text-right text-gray-900">{euro(row.currentRent)}</td>
